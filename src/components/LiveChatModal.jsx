...
export default function LiveChatModal({ onClose, openWhatsApp }) {
  ...
  return (
    <div className="fixed bottom-20 right-4 sm:right-8 z-50 w-full max-w-sm">
      <div className="bg-[#0b0e16] border border-[#00d2ff]/30 rounded-2xl shadow-2xl overflow-hidden flex flex-col h-[480px]">
        
        {/* Chat Header */}
        <div className="bg-[#101522] border-b border-white/10 p-3.5 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="relative">
              <div className="w-8 h-8 rounded-full bg-[#00d2ff]/20 border border-[#00d2ff]/40 flex items-center justify-center text-[#00d2ff]">
                <Headphones className="w-4 h-4" />
              </div>
              <span className="w-2.5 h-2.5 rounded-full bg-[#00d2ff] absolute -bottom-0.5 -right-0.5 border-2 border-[#101522]" />
            </div>
            <div>
              <h4 className="text-xs font-bold text-white font-mono">Diamond Care Desk</h4>
              <p className="text-[10px] text-slate-400">Live Specialist • 24/7 Active</p>
            </div>
          </div>
            ...
            >
              <div
                className={`max-w-[82%] rounded-2xl p-3 ${
                  msg.sender === 'user'
                    ? 'bg-[#00d2ff] text-[#07090e] font-medium rounded-br-none'
                    : 'bg-[#151c2c] text-slate-200 border border-white/10 rounded-bl-none'
                }`}
              >
        ...
        {/* Quick Prompts */}
        <div className="p-2 border-t border-white/5 bg-[#080b11] flex gap-1.5 overflow-x-auto no-scrollbar">
          {quickPrompts.map((qp, i) => (
            <button
              key={i}
              onClick={() => handleSend(qp)}
              className="px-2.5 py-1 rounded-lg bg-white/5 text-[10px] text-slate-300 hover:text-[#00d2ff] hover:bg-white/10 shrink-0 transition-colors cursor-pointer"
            >
              {qp}
            </button>
          ))}
        ...
        <div className="p-3 bg-[#0d121c] border-t border-white/10 flex items-center gap-2">
          <input
            ...
            className="flex-1 px-3 py-2 rounded-xl bg-[#141a26] border border-white/10 text-xs text-white placeholder-slate-500 focus:border-[#00d2ff] focus:outline-none"
          />
          <button
            onClick={() => handleSend()}
            className="p-2 rounded-xl bg-[#00d2ff] text-[#07090e] hover:bg-[#33d9ff] transition-colors cursor-pointer shrink-0"
          >
        ...
        <div className="p-2 bg-[#06080d] border-t border-white/5 text-center">
          <button
            onClick={() => openWhatsApp('Live Chat Escalation')}
            className="text-[11px] text-[#00d2ff] font-semibold hover:underline inline-flex items-center gap-1 cursor-pointer"
          >
